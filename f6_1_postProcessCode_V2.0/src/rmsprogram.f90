program rms
  ...
  REAL*8,ALLOCATABLE,DIMENSION(:,:,:)::urms,vrms,wrms,trms,prms,uv
  REAL*8,ALLOCATABLE,DIMENSION(:,:,:)::uprime,vprime,wprime,tprime,pprime
  REAL*8,ALLOCATABLE,DIMENSION(:,:,:)::uprime2,vprime2,wprime2,tprime2,pprime2,uv2
  ...
  ALLOCATE(wrms(n1,n2do+1,n3),vrms(n1,n2do+1,n3),&
       urms(n1,n2do+1,n3),trms(n1,n2do+1,n3),prms(n1,n2do+1,n3),uv(n1,n2do+1,n3))
  ! Read mean field
  ...
  DO ITime= INT(stTime),(INT(StTime)+(NumTimeSteps*Dt)),Dt
     ...
     ! printing fluctuating feild
     !write(time,'(i5.5)')itime
     
     !call sendrecv3dwrite(wprime,1,'wprime'//trim(time)//'.dat')
     !call sendrecv3dwrite(vprime,2,'vprime'//trim(time)//'.dat')
     !call sendrecv3dwrite(uprime,3,'uprime'//trim(time)//'.dat')  
     ...
     deallocate(tprime2,wprime2,vprime2,uprime2,pprime2,uv2)
  END DO
  etime=MPI_WTIME()
